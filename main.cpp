...
bool init() {
	...
	{
		gWindow = SDL_CreateWindow( "Minesweeper with SDL2", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, SCREEN_WIDTH, SCREEN_HEIGHT, SDL_WINDOW_SHOWN );
		if( gWindow == NULL )
		{
...
bool loadMedia() {
	...
	else
    {
		...
		if( !gPlayAgainWinTexture.loadFromRenderedText( "Press F to play again!", playAgainWin ) )
		{
			cout << "Failed to render text texture!\n";
			success = false;
		}
    ...
    {
		SDL_Color playAgainLose = { 0, 0, 0 };
		if( !gPlayAgainLoseTexture.loadFromRenderedText( "Press F to try again!", playAgainLose ) )
		{
			cout << "Failed to render text texture!\n";
			success = false;
		}
